...
def draw_guidelines(layout):
    ...
    for proportion in [1/8, 1/4, 1/2, 3/4, 7/8]:
        layout.canvas.line(
            0, layout.margin + layout.section_height * proportion,
            layout.width, layout.margin + layout.section_height * proportion
        )
...
def draw_margins(layout):
    ...
    if layout.margin:
        ...
        layout.canvas.line(layout.width / 2.0 - layout.margin, 0, layout.width / 2.0 - layout.margin, layout.height)
        layout.canvas.line(layout.width / 2.0 + layout.margin, 0, layout.width / 2.0 + layout.margin, layout.height)
        layout.canvas.line(0, layout.height / 2.0 + layout.margin, layout.width, layout.height / 2.0 + layout.margin)
        layout.canvas.line(0, layout.height / 2.0 - layout.margin, layout.width, layout.height / 2.0 - layout.margin)
        layout.canvas.setDash(1, 0)
